...
const SignInScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const startRotation = () => {
      ...
      Animated.timing(rotateValue, { toValue: 10, duration: 30000, useNativeDriver: true }).start(() => setTimeout(() => startRotation(), 30000));
    };
    startRotation();
  ...
  const animatedLogoStyle = { transform: [{ rotate: rotateInterpolate }] };
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const registeredAccounts = registeredAccountsJson ? JSON.parse(registeredAccountsJson) : [];
      let foundUser = registeredAccounts.find(u => u.email && u.email.toLowerCase() === email.trim().toLowerCase());
      
      if (!foundUser) {
        const saved = await AsyncStorage.getItem('userProfileData');
        if (saved) {
          const parsed = JSON.parse(saved);
          if (parsed.email && parsed.email.toLowerCase() === email.trim().toLowerCase()) foundUser = parsed;
        }
      }

      if (!foundUser) { Alert.alert('Account not found', 'No account matches this email. Please sign up first.'); return; }

      const storedPassword = foundUser.password || '';
      const hashedPassword = hashPassword(password);
      const isPasswordValid = storedPassword && (storedPassword === hashedPassword || storedPassword === password);
      
      if (!isPasswordValid) { Alert.alert('Invalid Password', 'The password you entered is incorrect.'); return; }
      
      if (storedPassword === password) {
        foundUser.password = hashedPassword;
        const userIndex = registeredAccounts.findIndex(u => u.email && u.email.toLowerCase() === foundUser.email.toLowerCase());
        if (userIndex !== -1) {
          registeredAccounts[userIndex] = foundUser;
          await AsyncStorage.setItem('registeredAccounts', JSON.stringify(registeredAccounts));
        }
      }

      await AsyncStorage.setItem('userProfileData', JSON.stringify(foundUser));
      
      // ✅ FIXED: Only clear social state. REMOVED 'savedCommissions' so data persists between logins.
      await AsyncStorage.multiRemove(['followingState', 'likesState']);

      Alert.alert('Success', 'You have successfully signed in!', [{ text: 'OK', onPress: () => navigation.navigate('Home') }]);
  ...
  return (
    <LinearGradient colors={isDarkMode ? gradients.background : gradients.main} locations={isDarkMode ? [0, 1] : [0, 0.58, 0.84]} style={{ flex: 1 }}>
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle={isDarkMode ? "light-content" : "dark-content"} backgroundColor="transparent" translucent />
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardAvoid}>
          <View style={styles.header}>
            <TouchableOpacity style={[styles.themeToggle, { backgroundColor: isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)' }]} onPress={toggleTheme}>
              <Ionicons name={isDarkMode ? 'sunny' : 'moon'} size={24} color={colors.primary} />
            </TouchableOpacity>
            <View style={styles.logoCircle}>
              <Animated.Image source={require('../../assets/lumivana.png')} style={[styles.logoCircle, animatedLogoStyle]} resizeMode="contain" />
            </View>
          </View>
          <View style={styles.content}>
            ...
            <View style={styles.form}>
              ...
              <TextInput style={[styles.input, { borderColor: colors.inputBorder || colors.primary, color: colors.inputText || colors.text, backgroundColor: colors.inputBackground || colors.surface }]} placeholder="Enter your email" placeholderTextColor={colors.inputPlaceholder || colors.textMuted} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />
              ...
              <View style={[styles.passwordContainer, { borderColor: colors.inputBorder || colors.primary, backgroundColor: colors.inputBackground || colors.surface }]}>
                <TextInput style={[styles.inputPassword, { color: colors.inputText || colors.text }]} placeholder="Enter your password" placeholderTextColor={colors.inputPlaceholder || colors.textMuted} value={password} onChangeText={setPassword} secureTextEntry={!showPassword} />
...
const styles = StyleSheet.create({
  ...
  themeToggle: { position: 'absolute', top: 60, right: 24, width: 40, height: 40, justifyContent: 'center', alignItems: 'center', borderRadius: 20, zIndex: 10 },
  logoCircle: { width: 90, height: 90, borderRadius: 45, borderWidth: 2, borderColor: '#FFD700', justifyContent: 'center', alignItems: 'center' },
  ...
  input: { borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, fontSize: 16, marginBottom: 8 },
  passwordContainer: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 12, marginBottom: 8, paddingHorizontal: 10 },
